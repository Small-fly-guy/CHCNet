...
class RandomHFlip(object):
    ...
    def __call__(self, img,img_box,mask, boxes, points):
        ...
        if flip_flag:
            ...
            points = points_temp
            return F.hflip(img), F.hflip(img_box),F.hflip(mask), boxes, points
        else:
            return img,img_box, mask, boxes, points
    ...
    def __call__(self, img,img_box, mask, boxes, points):
        ...
        if flip_flag:
            ...
            for point in points:
                ...
                points_temp.append([x, y])
            points = points_temp
            return F.vflip(img),F.vflip(img_box), F.vflip(mask), boxes, points
        else:
            return img,img_box, mask, boxes, points
